/*
  # Admin Table Separation Migration
  ...
  3. Security
    ...
    - Update policies for users, questions, and admins tables
    - Add helper functions for admin management

  4. Functions
    - `is_admin(user_id)` - Check if a user is an admin
    - `make_user_admin(user_id)` - Make a user an admin
    - `remove_user_admin(user_id)` - Remove admin privileges
*/
...
DROP FUNCTION IF EXISTS remove_user_admin(uuid);
